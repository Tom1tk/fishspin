...
function fmt(n) {
  ...
  if (n >= 10e3) return (n / 1e3).toFixed(2).replace(/\.?0+$/, '') + 'K';
  return String(n);
}
...
const SHOP_SECTIONS = [{
  ...
  items: [{
    ...
    name: 'Quick Auto',
    cost: 200,
    desc: 'Auto-spin cuts wheel animation at 1s — toggle on/off after purchase'
  }, {
    id: 'autospeed_2',
    emoji: '⏩',
    name: 'Rapid Auto',
    cost: 10000,
    desc: 'Auto-spin cuts wheel animation at 0.5s — toggle on/off after purchase',
